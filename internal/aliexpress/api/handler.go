...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if !strings.HasPrefix(r.URL.Path, "/v1/aliexpress/") {
		writeError(w, http.StatusNotFound, "not found")
		return
	}

	path := strings.Trim(strings.TrimPrefix(r.URL.Path, "/v1/aliexpress/"), "/")
	switch path {
	case "product":
		h.handleProduct(w, r)
	case "search":
...
func (h *Handler) handleProduct(w http.ResponseWriter, r *http.Request) {
	...
	var req scrape.ProductInput
	if err := decodeJSONBody(w, r, &req); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}

	result, err := h.service.ScrapeProduct(r.Context(), req)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *Handler) handleSearch(w http.ResponseWriter, r *http.Request) {
	...
	var req scrape.SearchInput
	if err := decodeJSONBody(w, r, &req); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}

	result, err := h.service.SearchProducts(r.Context(), req)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
